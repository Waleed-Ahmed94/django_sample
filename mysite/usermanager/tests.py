...
class LoginTest(TestCase):
    ...
    def test_no_user(self):
        ...
        response = self.client.post(reverse("usermanager:login"),
    ...
    def test_successful_login(self):
        ...
        self.assertRedirects(response, expected_url=reverse('usermanager:home'),
...
class SignupTest(TestCase):
    ...
    def test_user_already_exist(self):
        ...
        response = self.client.post(reverse("usermanager:create_user"),
                                    data={'username': 'user3', 'password':'1234', 'confirm-password':'1234'})
        self.assertContains(response, "username already exists")

    def test_password_not_match(self):

        response = self.client.post(reverse("usermanager:create_user"),
                                    data={'username': 'user4', 'password':'1234', 'confirm-password':'123'})
        self.assertContains(response, "password does not match")

    def test_successful_signup(self):

        response = self.client.post(reverse("usermanager:create_user"),
                                    data={'username': 'user4', 'password':'1234', 'confirm-password':'1234'})
